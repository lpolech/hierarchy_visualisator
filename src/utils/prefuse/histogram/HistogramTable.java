...
public class HistogramTable extends Table implements ColumnListener {
	...
	public HistogramTable(Table aTable, int aBinCount, int roleNumber) 
	{
		...
		for(int fieldIndex = 0; fieldIndex < fieldNames.length; fieldIndex++)//one deletion (one pass of this loop) allows to delete all empty bins,
	...
	public HistogramTable(Table aTable, /*int aBinCount,*/ HistogramTable computedBinsToUse, int roleNumber) {//REFACTOR: there is a duplication  
		super();//of code with above defined constructor
		...
		for (int binSlot = m_binCount-1; binSlot >= 0 && !foundNonZeroCount; binSlot--)
		{
			...
			if(currentCount == 0)
			{
					System.err.println("On " + field + " histogram, there is additional empty bin on the right or bottom side."
						+ " In odrer to remove it, decrease number of bins. Honestly, this should never happened.");
			}
			else
			{
